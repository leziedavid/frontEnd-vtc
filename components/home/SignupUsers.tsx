...
const userSchema = z.object({
    ...
    password: z.string()
        .optional()
        .refine(val => {
            if (!val) return true; // 🔹 si vide, OK
            return passwordRegex.test(val); // sinon doit respecter la regex
        }, "Le mot de passe doit contenir 8 caractères minimum, avec une majuscule, un chiffre et un symbole"),
    ...
    .refine((data) => {
        // 🔹 Vérifie le confirmPassword uniquement si password est fourni
...
export default function SignupUsers({ role, initialValue, onSubmit }: SignupUsersProps) {
    ...
    return (
        <form onSubmit={handleSubmit(submitHandler)} className="text-slate-600 flex flex-col w-full max-w-3xl mx-auto p-6 lg:p-10 gap-6" >
            <h1 className="text-2xl lg:text-3xl font-semibold text-slate-800 uppercase">
                {initialValue?.id ? `Modifier un compte ${roleFR[role]}` : `Créer un compte ${roleFR[role]}`}
            </h1>
