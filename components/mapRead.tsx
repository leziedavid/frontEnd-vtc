...
import { Clock, MapPin, Navigation, ChevronDown, ChevronUp, Flag, CheckCircle } from "lucide-react"
...
export default function MapRead({ trajetId, trajetData, onFinish }: MapReadProps) {
    ...
    // Fonction pour terminer manuellement la course
    const handleManualFinish = () => {
        if (window.confirm("Êtes-vous sûr de vouloir terminer la course maintenant ?")) {
            setProgress(100)
            if (onFinish) {
                onFinish()
            }
    ...
    return (
        <div className="flex flex-col h-screen bg-gray-50">
            ...
            <div className="flex-1 relative bg-gray-100 overflow-hidden">
                ...
                {/* Bouton flottant pour terminer manuellement */}
                {showFinishButton && (
                    <div className="absolute bottom-24 right-4 flex flex-col gap-2 z-50">
                        <button
                            onClick={handleManualFinish}
                            className="bg-red-600 hover:bg-red-700 text-white rounded-full p-4 shadow-2xl transition-all duration-300 flex items-center justify-center group"
                            title="Terminer la course manuellement"
                        >
                        ...
                        {/* Bouton pour cacher le bouton flottant */}
                        <button
                            onClick={() => setShowFinishButton(false)}
                            className="bg-gray-600 hover:bg-gray-700 text-white rounded-full p-2 shadow-lg transition-colors"
                            title="Cacher le bouton"
                        >
            ...
            <div className={`bg-white rounded-t-3xl shadow-2xl transition-all duration-300 relative ${isPanelVisible ? 'p-6' : 'p-0 h-0 overflow-hidden' }`}  >
                {/* Bouton pour réafficher le bouton flottant quand il est caché */}
                {!showFinishButton && (
                    <button onClick={() => setShowFinishButton(true)} className="absolute -top-12 right-6 bg-gray-600 hover:bg-gray-700 text-white rounded-full p-3 shadow-lg transition-colors z-10 flex items-center gap-2"  title="Afficher le bouton de fin de course" >
                ...
                <div className="space-y-4">
                    ...
                    <button
                        ...
                        className={`w-full py-3 rounded-xl font-semibold transition-all flex items-center justify-center gap-2 ${progress >= 100
                                ? 'bg-green-600 hover:bg-green-700 text-white shadow-lg'
                                : 'bg-gray-200 text-gray-400 cursor-not-allowed'
                            }`}
                    >
                        {progress >= 100 ? (
                            <>  <CheckCircle className="w-5 h-5" /> ✓ Terminer la course
                            </>
                        ) : (
                            'Course en cours...'
                        )}
                    ...
                    {/* Note pour la fin manuelle */}
                    <div className="text-center text-xs text-gray-500">
                        <p>Vous pouvez terminer la course manuellement à tout moment avec le bouton drapeau</p>
                    </div>
                </div>
            </div>
        </div>
