...
export default function PageTrajet() {
  ...
  // const [mapChargee, setMapChargee] = useState(true)
  ...
  const demanderGeolocalisation = () => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        {
          enableHighAccuracy: false, // Utiliser false pour une réponse plus rapide
          timeout: 10000, // 10 secondes maximum
          maximumAge: 300000 // Accepter une position en cache de 5 minutes
        }
      )
    } else {
      utiliserLocalisationParDefaut()
  ...
  useEffect(() => {
    const checkLocationPermission = async () => {
      ...
      if (permissionGranted === 'true') {
        // L'utilisateur a déjà accepté, demander directement
  ...
  const handleRechercher = async () => {
    ...
    try {
      ...
      if (res.data) {
        ...
        // currentPage est déjà géré par useEffect
      }
  ...
  const handleReserver = () => {
    setOpenConfirm(true)
    if (!trajetSelectionne) return
    // alert(`✅ Réservation confirmée : ${trajetSelectionne.departure} → ${trajetSelectionne.destination} (${places} place${places > 1 ? "s" : ""})`)
  }
  ...
  const handleConfirmerCommande = async () => {
    ...
    const commandeDto = {
      ...
      price: (trajetSelectionne.price + (selectedType?.price ?? 0)) * places, // Calcul du prix total avec nombre de places
      status: "PENDING",
    }
    try {
      ...
      if (response.statusCode === 201) {
        ...
        setPlaces(1) // reset nombre de places
      } else {
  ...
  useEffect(() => {
    setLocalisationUtilisateur({ lat: 5.36, lng: -4.0083 }) // Abidjan
  }, [])

  // 🔹 Recherche automatique quand la page change
  useEffect(() => {
    if (!depart || !destination) return; // si départ/destination non renseignés, ne rien faire
    handleRechercher();
  }, [currentPage]);
        ...
        <div className="flex flex-col lg:flex-row h-full">
          ...
          <div className="w-full lg:w-2/3 h-[300px] lg:h-auto relative bg-gray-200 dark:bg-gray-800">
            {!mapChargee ? (
              ...
              <SimpleMap
                userLocation={localisationUtilisateur}
                destination={localisationDestination}
                onLocationChange={(loc) => {
                  setLocalisationUtilisateur(loc)
                  setDepart("Position actuelle")
                }}
              />
            )}
