...
import { cancelCommande } from "../services/commandeService";
import { getUserOrders } from "../services/securityService";
...
export default function Page() {
  ...
  // const [commandes, setCommandes] = useState<Commande[]>(fakeCommandes);
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      if (data.statusCode === 200) {
        ...
        setSelectedCommande((prev: Commande | null) =>
          prev ? { ...prev, status: CommandeStatus.CANCELLED } : prev
        );
      }
    } catch (error) {
    ...
    <>
      ...
      <div className="p-6 lg:p-10 max-w-7xl mx-auto relative">
        ...
        {loading ? (
          ...
          <DriverCourses
            commande={commandes} onSelect={(com) => { setSelectedCommande(com); setDrawerOpen(true); }}
          ...
          <div className="relative bg-white w-full sm:w-3/4 md:w-2/3 lg:w-1/2 xl:w-2/5 h-auto sm:rounded-lg overflow-auto transform animate-slide-in p-6 flex flex-col gap-4 shadow-lg">
            ...
            <div className="border-2 border-gray-200 rounded-lg p-4 bg-gray-50 shadow-inner">
              ...
              <div className="flex justify-end gap-2 mt-2">
                {/* Bouton fermer */}
                <button onClick={() => setDrawerOpen(false)} className="py-2 px-4 rounded-md border border-gray-300 bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors" >
                  Fermer
                </button>

                {/* Bouton annuler la commande */}
                <button onClick={handleCancel} disabled={selectedCommande.status === CommandeStatus.CANCELLED} className={`py-2 px-4 rounded-md border transition-colors ${selectedCommande.status === CommandeStatus.CANCELLED ? "border-gray-300 bg-gray-100 text-gray-400 cursor-not-allowed" : "border-red-500 bg-red-500 text-white hover:bg-red-600"}`} >
                  Annuler ma commande
                </button>
              </div>
