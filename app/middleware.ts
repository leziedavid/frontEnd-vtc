...
import { Role, UserStatus } from '@/types/interfaces'
...
export const getUserInfos = async (): Promise<{ id: string; role: Role; partnerId: string | null , imageUrl: string | null, name: string | null } | null> => {
    const user = await useAuthMiddleware()
    if (!user) return null
    return {
        id: user.sub,
